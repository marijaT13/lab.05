...
char tocniPrvi[4] = { 'b', 'c', 'a', 'c' };
char tocniVtori[4] = { 'a', 'd', 'd', 'd' };
char tocniTreti[4] = { 'b', 'e', 'a', 'd' };
...
void writeOdgovor(fstream& file, fstream& log, char odg, int br) {
    file << br << ". " << odg <<endl;
    log<< br << ". " << odg <<endl;
}
...
float handleInput(int brojprasanje, fstream& file, fstream& log) {
    ...
    if (brojprasanje <= 4) {
        ...
        writeOdgovor(file, log, odgovor, brojprasanje);
...
int main() {
	//pokazuvac na vreme i data koga zapocnuva odgovaranjeto na igracot
 			 	time_t rawtime;
  				struct tm * timeinfo;
				  time (&rawtime);
				  timeinfo = localtime (&rawtime);
				  printf ("Deneshnata data i segashnoto vreme e: %s", asctime(timeinfo));
    ...
    //kreiranje na login file za sekoj igrac
    ...
    while (getline(test, prasanje)) {
        ...
        poeni += handleInput(brojnaprasanje, file, log);
    ...
    answersTable();
        ...
        cout<<"Vekje imate odgovoreno na prasanjata!"<<endl;
    }
}
